theme: /CheckBalance
    ...
    state: Balance
        ...
        if: $session.userAuthorized == true
            script:
                fetchBalanceCustomerData(
                    ...
                    var account = customer.id || "неизвестно";
                    ...
                    if (debt > 0) {
                        $reactions.answer(
                          "Номер Вашего лицевого счёта: " + account + "\n" +
                          "ФИО: " + name + "\n" +
                          "Адрес: " + address + "\n" +
                          "На Вашем счете есть задолженность в размере " + declineSom(debt) + ". Пополните его для восстановления доступа к услугам. Рассказать о способах оплаты?"
                        );
        ...
        state: Again
            q: $regexp_i<(выдайте\s+кредит\s+ещё\s+раз|дайте\s+временный\s+доступ\s+снова|предоставьте\s+доступ\s+повторно|снова\s+получить\s+доступ|повторно\s+выдать\s+кредит)>
            go!: /CheckYourDebt
    ...
    state: PaymentMethods
        a: 
            ...
            💻 На нашем сайте: 
            - [aknet](https://aknet.kg/oplata_uslug/)
